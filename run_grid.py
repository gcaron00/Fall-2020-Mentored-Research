...
os.mkdir('./run_grid_nets/nets_{0:s}'.format(dt))
...
Coronal_controls = np.fliplr(np.flipud(c_t2[opts['zz']:opts['zz']+opts['siz'], 
                                            opts['jj'], opts['tt']:opts['tt']+opts['siz'], :]))
...
y = np.concatenate((np.ones(Coronal_controls.shape[0], dtype = int), 
                    np.zeros(Coronal_patients.shape[0], dtype = int)), axis=0)
...
for j in range(0, len(S['ytrain'])):
    ...
    net.fit(x=S['xtrain'][j], y=S['ytrain'][j], epochs=opts['mx_epochs'], shuffle=True, 
            validation_freq=opts['val_freq'], validation_data=(S['xval'][j], S['yval'][j]), 
    ...
    net.save('./run_grid_nets/nets_{0:s}/net_{1:s}'.format(dt, dt2))

f = open('./run_grid_opts/gs_opts_{0:s}.pkl'.format(dt),"wb")
pickle.dump(opts, f)
f.close() 
